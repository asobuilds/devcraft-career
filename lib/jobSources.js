...
async function fetchTheMuseJobs(keywords) {
  ...
  const url = 'https://www.themuse.com/api/public/jobs?page=1&descending=true&query=' + query;
  const data = await safeFetchJson(url);
  if (!data || !data.results) return [];
  ...
  return data.results.map((job) => ({
    source: 'The Muse',
    title: job.name,
    company: job.company && job.company.name ? job.company.name : 'Unknown Company',
    location:
...
async function fetchSmartRecruitersJobs(keywords) {
  const query = encodeURIComponent(keywords.slice(0, 2).join(' '));
  const url = 'https://api.smartrecruiters.com/v1/postings?q=' + query + '&limit=20';
  const data = await safeFetchJson(url);
...
export async function findMatchingJobs(techStackString, options) {
  ...
  const results = await Promise.all([
    ...
    fetchSmartRecruitersJobs(keywords),
  ]);
