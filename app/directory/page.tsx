...
export default function RecruiterDirectory() {
  ...
  // Normalize tech_stack to an array of strings
  ...
  // Filtering logic – safe for arrays
  const filteredPortfolios = portfolios.filter(p => {
    const searchLower = searchTerm.toLowerCase().trim();
    if (!searchLower) return true;

    const techArray = getTechArray(p.tech_stack);
    const matchStack = techArray.some(t => t.toLowerCase().includes(searchLower));
    const matchBio = p.bio?.toLowerCase().includes(searchLower) || false;
    const matchTitle = p.title?.toLowerCase().includes(searchLower) || false;

    return matchStack || matchBio || matchTitle;
  });
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12 font-sans">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="border-b border-slate-900 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-white">Reverse Engineer Talent Board</h1>
            <p className="text-xs text-slate-500 mt-1">Browse verified active candidate profiles and live project code streams instantly.</p>
          </div>
          ...
          <div className="relative w-full md:w-80">
            ...
            <input 
              ...
              placeholder="Filter by tech (e.g. React, SQL, Python)..." 
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPortfolios.map((p) => {
            const techArray = getTechArray(p.tech_stack);
            const displayName = p.title || 'Untitled Portfolio';
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-400 line-clamp-3 text-justify leading-relaxed">
                      {p.bio || "No professional profile biography synchronized yet."}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4 mt-6 pt-4 border-t border-slate-900">
                  {techArray.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {techArray.slice(0, 4).map((tech: string, idx: number) => (
        ...
        {filteredPortfolios.length === 0 && (
          <p className="text-center text-xs text-slate-700 italic py-20">
            No matching technical dossiers met current search filter matrix rules parameters.
          </p>
        )}
      </div>
    </div>
